...
.EXAMPLE
    ...
    Deep pass: recursively SHA-256 recovered folder contents, skipping files over 100 MB.

.EXAMPLE
    .\hunt_recyclebin.ps1 -Since 2026-07-20 -MinSeverity High
    Incident window: filters to items deleted on/after this date, activates DELETION-CLUSTER,
    and shows only HIGH-tier anomalies in the queue.

.EXAMPLE
    .\hunt_recyclebin.ps1 -Path 'E:\mounted_image' -User jdoe
    Point at a mounted/offline image's Recycle Bin (in addition to live volumes) and scope
    results to one owner.
...
Write-Host "    Inaccessible roots/SID folders (elevation or removed mid-scan) : $($InaccessibleRoots.Count)"
