...
.EXAMPLE
  ...
  Deep-dive a single suspect shortcut - full COM properties plus MS-SHLLINK forensic metadata.
.EXAMPLE
  hunt_lnk.ps1 -OutFile C:\triage\lnk_findings.csv -Hash -Detail
  Full verbose sweep plus a CSV export, hashing the resolved target of every flagged item.
.NOTES
...
foreach ($j in $jb) {
  ...
  foreach ($e in $localErr) {
    ...
    if (Test-IsJunction -Path $deniedPath) { continue }
    # Not every walk error is a permissions problem -- a broken/dangling symlink (WSL rootfs
    # paths under snapd/systemd are a common real-world source) throws DirectoryNotFoundException,
    # not UnauthorizedAccessException. Lumping both into "access-denied" and then telling the
    # operator to "re-run elevated" is actively wrong advice for a path that doesn't exist at all.
    if ($e.CategoryInfo.Category -eq 'PermissionDenied' -or $e.Exception -is [System.UnauthorizedAccessException]) {
      $dp.Add($deniedPath)
    } else {
      $brokenPaths.Add("$deniedPath ($($e.Exception.GetType().Name))")
    }
  }
}
...
Write-Host "`n-- other unreadable paths skipped, not a permissions issue ($($brokenPaths.Count)) --"
if ($brokenPaths.Count -eq 0) {
  Write-Host '   (none)'
} else {
  foreach ($b in $brokenPaths) { Write-Host "   $b" }
}
Write-Host "`n==== $($high.Count) HIGH  $($nt.Count) NOTABLE  $($cl.Count) clean  $($ur.Count) unreadable  $($dp.Count) access-denied  $($brokenPaths.Count) other-unreadable ===="
