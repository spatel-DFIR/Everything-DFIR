...
## 3. The MaxEvents bugs this rewrite fixes

### Bug 1: raw ordering (fixed at the original rewrite)
...
`-MaxEvents` trims the result set.

### Bug 2: keyword starvation (found testing this rewrite)

Even with the timeframe applied first, capping **raw event retrieval** at `-MaxEvents` before
keyword/pattern matching has the exact same silent-miss effect one level down: the newest N raw
events in the window can easily contain zero mentions of the keyword, while hundreds of matches
sit further back in the same window — and the operator has no way to tell "genuinely no matches"
from "your cap ate them." This is especially easy to hit on a chatty log
(`Microsoft-Windows-PowerShell/Operational` with script-block logging on can log 3-4 events per
command) combined with a small explicit `-MaxEvents`.

**The fix:** when `-Keywords`/`-Pattern` is active, retrieval is never capped — the full
timeframe is always searched — and `-MaxEvents` instead caps how many **matches** are shown per
log (newest first), with an honest "N more matches exist" note in the summary if the cap was
hit. A keyword/pattern search additionally always shows **at least the newest 50 matches** per
log even if `-MaxEvents` was explicitly set lower, so an accidentally tiny cap can't hide real
results. A scope-only dump (no `-Keywords`/`-Pattern`) is unaffected by either fix above — there
every event in scope already **is** a "match," so capping retrieval and capping displayed
results are the same operation, and `-MaxEvents` is honored exactly as given.
...
5. **Reach for `-Pattern` when a keyword can't express the shape you want** — e.g.
   `-Pattern '(?i)(-enc\b|-EncodedCommand|IEX\b|Invoke-Expression|DownloadString|DownloadFile)'`
   for obfuscated/download-cradle indicators inside a PowerShell script-block body, or
   `-Pattern '\\Users\\[^\\]+\\AppData\\Local\\Temp\\.*\.(exe|dll|ps1)$'` for an
   ...
   **`-Keywords`/`-Pattern` match the event's `Message` *body* text only, never the EventID.**
   A pattern like `\b4104\b` looks like it should find "4104" (script-block-logging) events, but
   won't: a 4104 event's own Message is the script block's *content*, which essentially never
   contains the literal digits of its own event ID. To scope to specific event types, use
   `-LogName`/`-Level`, or match a phrase that genuinely appears in the body text you're after.
...
| `-MaxEvents <int>` | Scope-only dump: per-log cap on returned events, applied **after** the timeframe filter. Keyword/pattern search: per-log cap on **matches shown** (newest first) — the full timeframe is always searched first, and at least the newest 50 matches per log are always shown even if this is set lower (default `500`) |
